...
export default function QuantityControl({ quantity, onChange, min = 0, size = 'sm' }: Props) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <button
        onClick={() => onChange(quantity + 1)}
        className={`${btnSize} rounded-full border border-gold/30 flex items-center justify-center text-gold hover:bg-gold/10 transition-colors`}
      >
        <Plus size={iconSize} />
      </button>
    </div>
